...
{
    short int chromosome[8]; //染色体编码，此处为5位二进制
    double fit;
    double rfit; //适应值比例
    double cfit; //累计适应值
} chrom;
//定义初始种群，下一代种群
chrom population_current[100];
chrom population_nextgene[100];
double pr = 0.85;
double px = 0.05;
...
int main()
{
    ...
    cout << "The population of the chroms is 100." << endl;
    return 0;
}
...
void *cross_chroms(chrom population_nextgene[30])
{
    ...
    uniform_int_distribution<unsigned> p(0, 7);
    ...
    do
    {
        ...
        if (t < pr)
        {
            for (j = 4; j < 8; j++)
            {
                a = population_nextgene[i].chromosome[j];
                population_nextgene[i].chromosome[j] = population_nextgene[i - 1].chromosome[j];
            }
        }
        i += 2;
